    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                # Check that the User.risk_score is one of the value in the
                # Enum UserRiskScoreType --> ['No risk', 'Low', 'Medium', 'High']
                condition=models.Q(
...
class Alert(models.Model):
    ...
    tags = ArrayField(
        ...
        default=list,
        help_text=(
            "List of Tags used to classify the alert. " "Multiple tags can be applied."
        ),
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                # Check that the Alert.name is one of the value in the Enum
                # AlertDetectionType
                condition=models.Q(
            ...
            models.CheckConstraint(
                # Check that each element in the Alert.filter_type is in the
...
class Config(models.Model):
    ...
    ignored_users = ArrayField(
        ...
        help_text=(
            "List of users (strings or regex patterns) to be ignored "
            "from the detection"
        ),
    )
    enabled_users = ArrayField(
        ...
        validators=[validate_string_or_regex],
        help_text=(
            "List of selected users (strings or regex patterns) on which "
            "the detection will perform"
        ),
    )
    ...
    alert_minimum_risk_score = models.CharField(
        ...
        help_text=(
            "Select the risk_score that users should have at least to "
            "send the alerts"
        ),
    )
    ...
    allowed_countries = ArrayField(
        ...
        help_text=(
            "List of countries to exclude from the detection, because "
    ...
    threshold_user_risk_alert = models.CharField(
        ...
        default=UserRiskScoreType.MEDIUM,
        help_text=(
            "Select the risk_score that a user should overcome to send "
            "the 'USER_RISK_THRESHOLD' alert"
        ),
    )
    ignored_impossible_travel_countries_couples = models.JSONField(
        ...
        validators=[validate_countries_names],
        help_text=(
            "List of country pairs (start_country, last_country) to ignore "
            "for impossible_travel alerts. Country names must match the names "
            "in the countries_list.json config file. "
    ...
    ignored_impossible_travel_all_same_country = models.BooleanField(
        default=True,
        help_text=(
            "If true, all the impossible travel alerts from and to the same "
            "country are ignored. If you want to exclude just some countries, "
            "use the 'ignored_impossible_travel_countries_couples' Config "
            "field instead"
        ),
    )
    ...
    distance_accepted = models.PositiveIntegerField(
        ...
        help_text=(
            "Minimum distance (in Km) between two logins after which the "
            "impossible travel detection starts"
        ),
    )
    vel_accepted = models.PositiveIntegerField(
        default=settings.CERTEGO_BUFFALOGS_VEL_TRAVEL_ACCEPTED,
        help_text=(
            "Minimum velocity (in Km/h) between two logins after which the "
            "impossible travel detection starts"
        ),
    )
    ...
    user_learning_period = models.PositiveIntegerField(
        ...
        help_text=(
            "Days considered to learn the user login behaviors - no alerts "
            "generation"
        ),
    )
    ...
    def clean(self):
        if not self.pk and Config.objects.exists():
            raise ValidationError(
                "A Config object already exist - it is possible just to "
                + "modify it, not to create a new one"
            )
        # Config.id=1 always
        self.pk = 1
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                # Check that the Config.alert_minimum_risk_score is one of the
                # value in the Enum UserRiskScoreType
                condition=models.Q(
                    risk_score__in=[choice[0] for choice in UserRiskScoreType.choices]
                ),
            ...
            models.CheckConstraint(
                # Check that each element in the Config.filtered_alerts_types
                # is blank or it's in the Enum AlertFilterType
                condition=models.Q(
