...
def update_risk_level(
    ...
    with transaction.atomic():
        ...
        if risk_comparison in [ComparisonType.LOWER, ComparisonType.EQUAL]:
            logger.info(
                f"The current user.risk_score ({current_risk_score}) is "
                f"{risk_comparison.value} than the new risk_score: "
                f"{new_risk_level}. The Config.risk_score_increment_alerts list "
                f"contains: {app_config.risk_score_increment_alerts}"
            )
            return False  # risk_score doesn't increased, no USER_RISK_THRESHOLD
        ...
        if config_threshold_comparison in [ComparisonType.EQUAL, ComparisonType.HIGHER]:
            alert_info = {
                ...
                "alert_desc": (
                    f"{AlertDetectionType.USER_RISK_THRESHOLD.label} for User: "
                    f"{db_user.username}, who changed risk_score from "
                    f"{current_risk_score} to {new_risk_level}"
                ),
            }
            logger.info(
                f"Upgraded risk level for User: {db_user.username} to level: "
                f"{new_risk_level}, detected {db_user.alert_set.count()} "
                "alerts. The Config.risk_score_increment_alerts list contains: "
                f"{app_config.risk_score_increment_alerts}"
            )
            set_alert(
                db_user=db_user,
...
def set_alert(
    ...
    """
    logger.info(
        f"ALERT {alert_info['alert_name']} for User: {db_user.username} at: "
        f"{login_alert['timestamp']}"
    )
    ...
    for login in fields:
        if login.get("intelligence_category", None) == "anonymizer":
            ...
            alert_info = {
                ...
                "alert_desc": (
                    f"{AlertDetectionType.ANONYMOUS_IP_LOGIN.label} from IP: "
                    f"{login['ip']} by User: {db_user.username}"
                ),
            }
            set_alert(
...
def check_country(db_user: User, login_field: dict, app_config: Config) -> dict:
    ...
    if db_user.login_set.filter(country=login_field["country"]).count() == 0:
        ...
        alert_info["alert_desc"] = (
            f"{AlertDetectionType.NEW_COUNTRY.label} for User: "
            f"{db_user.username}, at: {login_field['timestamp']}, from: "
            f"{login_field['country']}"
        )
    # check "Atypical Country" alert
        ...
        alert_info["alert_desc"] = (
            f"{AlertDetectionType.ATYPICAL_COUNTRY.label} for User: "
            f"{db_user.username}, at: {login_field['timestamp']}, from: "
            f"{login_field['country']}"
        )
    return alert_info
...
def update_model(db_user: User, new_login: dict):
    ...
    try:
        ...
        logger.error(
            f"Unexpected error while updating a previous login in the DB for "
            f"the User: {db_user.username} with the new login (event_id: "
            f"{new_login['id']}): {e}"
        )
...
def calc_distance_impossible_travel(
    ...
    if distance_km > app_config.distance_accepted:
        ...
        if vel > app_config.vel_accepted:
            ...
            alert_info["alert_desc"] = (
                f"{AlertDetectionType.IMP_TRAVEL.label} for User: "
                f"{db_user.username}, at: "
                f"{last_login_user_fields['timestamp']}, from: "
                f"{last_login_user_fields['country']}, previous country: "
                f"{prev_login.country}, distance covered at {int(vel)} Km/h"
            )
