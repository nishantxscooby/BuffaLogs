...
def match_filters(alert: Alert, app_config: Config) -> Alert:
    ...
    if alert.login_raw_data.get("ip", "") in app_config.ignored_ips:
        logger.debug(
            f"Alert: {alert.id} filtered for user: {db_user.username} because "
            f"the login IP: {alert.login_raw_data['ip']} is in the ignored_ips "
            "Config list"
        )
    ...
    if alert.login_raw_data.get("country", "") in app_config.allowed_countries:
        logger.debug(
            f"Alert: {alert.id} filtered for user: {db_user.username} because "
            f"the login IP: {alert.login_raw_data['country']} is in the "
            "allower_countries Config list"
        )
        alert.filter_type.append(
    ...
    if alert.login_raw_data.get("organization", "") in app_config.ignored_ISPs:
        logger.debug(
            f"Alert: {alert.id} filtered for user: {db_user.username} because "
            f"the login ISP: {alert.login_raw_data['organization']} is in the "
            "ignored_ISPs Config list"
        )
    ...
    if app_config.ignore_mobile_logins and alert.login_raw_data.get("agent", ""):
        ...
        if (
            ...
            logger.debug(
                f"Alert: {alert.id} filtered for user: {db_user.username} "
                f"because the login user-agent: {alert.login_raw_data['agent']} "
                "is a mobile device and Config.ignore_mobile_logins: "
                f"{app_config.ignore_mobile_logins}"
            )
    ...
    if alert.name == AlertDetectionType.IMP_TRAVEL:
        # check ignored_impossible_travel_countries_couples and
        # ignored_impossible_travel_all_same_country config filters
        if (
        ...
        # using Counter to ignore the order:
        # ["Italy", "Germany"] == ["Germany", "Italy"] and check only if the
        # coutry couple is present in the ignored couples
        for (
...
def _update_users_filters(db_alert: Alert, app_config: Config, db_user: User) -> Alert:
    ...
    if app_config.alert_is_vip_only:
        ...
        if db_user.username not in app_config.vip_users:
            logger.debug(
                f"Alert: {db_alert.id} filtered because user: "
                f"{db_user.username} not in vip_users and enabled_users Config "
                "lists"
            )
        ...
        if app_config.enabled_users and not _check_username_list_regex(
            ...
            logger.debug(
                f"Alert: {db_alert.id} filtered because user: "
                f"{db_user.username} not in the enabled_users Config list"
            )
            ...
            if _check_username_list_regex(
                ...
                logger.debug(
                    f"Alert: {db_alert.id} filtered because user: "
                    f"{db_user.username} is in the ignored_users Config list"
                )
                db_alert.filter_type.append(AlertFilterType.IGNORED_USER_FILTER)
    ...
    if (
        ...
        logger.debug(
            f"Alert: {db_alert.id} filtered because user: {db_user.username} "
            f"has risk_score: {db_user.risk_score}, but "
            "Config.alert_minimum_risk_score is set to "
            f"{app_config.alert_minimum_risk_score}"
        )
    ...
    if (now - db_user.created) < timedelta(days=app_config.user_learning_period):
        logger.debug(
            f"Alert: {db_alert.id} filtered because user: {db_user.username} "
            "is still into the learning period"
        )
