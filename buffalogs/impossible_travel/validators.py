...
def validate_countries_names(values):
    ...
    if invalid_entries:
        raise ValidationError(
            _("The following country codes are invalid: %(invalid_entries)s")
            % {"invalid_entries": ", ".join(map(str, invalid_entries))}
        )
...
def validate_tags(values):
    ...
    if invalid:
        raise ValidationError(
            _("Invalid tags: %(invalid)s. Must be one of: %(valid_tags)s.")
            % {
...
def validate_boolean_str(value: Optional[str] = None):
    ...
    if str(value).lower() == "false":
        return False
    raise ValidationError(
        f"Notification status must be true or false, got {value}",
    )
...
def validate_alert_query(query_dict: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        msg = "Limit/Offset must be valid integers or numeric strings"
