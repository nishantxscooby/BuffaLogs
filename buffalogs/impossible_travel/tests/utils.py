...
def load_test_data(name: str) -> dict[str, Any]:
    """Utility for returning the data for tests in the tests/test_data
    folder"""
    with open(
        file=os.path.join(
...
def patched_components(
    ...
    """
    Dynamic/Flexible Patch flessibile for components used in the
    process_logs function.
        - Ingestion patch: patch_ingestion=True/False with optional return
          values: users_return, logins_return, normalized_return
    ...
    if patch_tasksettings:
        p_task = patch(
            "impossible_travel.tasks.process_logs.TaskSettings.objects." "get_or_create"
        )
    ...
    try:
        if patch_ingestion:
            ...
            ingestion_factory_mock.return_value.get_ingestion_class.return_value = (
                ingestion_mock
            )

        if patch_detection:
            detection_mock = (
                active_patches[-1] if patch_ingestion else active_patches[0]
            )
